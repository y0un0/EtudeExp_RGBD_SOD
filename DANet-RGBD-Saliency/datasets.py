...
def make_dataset(root):
    ...
    while line:
        ...
        d_list.append(os.path.split(a[1])[-1][:-4])
        gt_list.append(os.path.split(a[2])[-1][:-4])
        line = f.readline()
    # print(d_list)
    f.close()
    ...
    return [(os.path.join(img_path, img_name + '.jpg'),
             os.path.join(depth_path,d_name + '.png'),os.path.join(gt_path, gt_name + '.png')) for img_name, d_name, gt_name in zip(img_list, d_list, gt_list)]
