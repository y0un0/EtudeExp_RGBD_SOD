...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            s, s_d = model(images,depths)
            loss1 = F.binary_cross_entropy_with_logits(s, gts)
            # loss2 = F.binary_cross_entropy_with_logits(s_r, gts)
            loss3 = F.binary_cross_entropy_with_logits(s_d, gts) # Comment it later
            loss = loss1 + loss3 # Suppress loss3 later
            loss.backward()
...
if __name__ == '__main__':
    ...
    # load data
    print('load data...')
    train_loader = get_loader(root, batchsize=opt.batchsize, trainsize=opt.trainsize)
