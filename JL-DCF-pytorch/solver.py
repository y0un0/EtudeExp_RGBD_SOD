    ...
    def test(self):
        ...
        for i, data_batch in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                time_s = time.time()
                preds, pred_coarse, feat = self.net(input)
                torch.cuda.synchronize()
                time_e = time.time()
                ...
                # Visualisation des features map de la branche rgb
                # et de la branche depth
                enc = 0
                for f in feat:
                  enc += 1
                  for i in range(16):
                      plt.subplot(2, 8, i + 1)
                      fmr = f[0][i].cpu().detach().numpy()
                      plt.imshow(fmr, cmap='plasma')
                      plt.axis("off")
                  plt.savefig('feat_rgb'+str(enc)+'_JLDCF.jpg', dpi=300)
                  for j in range(16):
                      plt.subplot(2, 8, i + 1)
                      fmd = f[0][j].cpu().detach().numpy()
                      plt.imshow(fmd, cmap='plasma')
                      plt.axis("off")
                  plt.savefig('feat_d'+str(enc)+'_JLDCF.jpg', dpi=300)
